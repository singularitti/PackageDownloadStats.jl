...
!!! info
    The style guide is always a work in progress, and not all PackageDownloadStats code
    follows the rules. When modifying PackageDownloadStats, please fix the style violations
...
PackageDownloadStats uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/PackageDownloadStats.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the PackageDownloadStats directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to PackageDownloadStats have
