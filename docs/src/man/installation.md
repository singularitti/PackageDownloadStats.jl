...
[PackageDownloadStats](https://github.com/singularitti/PackageDownloadStats.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("PackageDownloadStats")
   ```

3. Run

   ```julia-repl
   julia> using PackageDownloadStats
   ```
...
version of PackageDownloadStats, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/PackageDownloadStats.jl"
```
...
our [GitHub repository](https://github.com/singularitti/PackageDownloadStats.jl)
for new releases.
Once we release a new version, you can update PackageDownloadStats by typing

```@repl
using Pkg
Pkg.update("PackageDownloadStats")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("PackageDownloadStats")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall PackageDownloadStats.
